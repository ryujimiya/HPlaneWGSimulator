...
namespace KrdLab.Lisys.Method
{
    ...
    public class EigenvalueDecomposition
    {
        ...
        /// </exception>
        /// <remarks>
        /// 内部では，<paramref name="X"/>がコピーされてから<see cref="KrdLab.clapack.Function.dgeev(double[], int, int, ref double[], ref double[], ref double[][], ref double[][])"/>に渡される．
        /// </remarks>
        public EigenvalueDecomposition(Matrix X)
        {
            ...
            clapack.Function.dgeev(tmp._body, tmp._rsize, tmp._csize,
            ...
            {
                if (!clapack.CalculationChecker.IsLessThanLimit(elem))
                {
